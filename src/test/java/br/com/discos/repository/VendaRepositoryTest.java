...
public class VendaRepositoryTest {
	...
	@Test
	public void quandoExcluirVenda_ExcluirItensVenda_ManterDiscoCashback() throws Exception {
		Venda venda = vendas.get(0);
		assertDoesNotThrow(() -> vendaRepository.deleteById(venda.getCodigo()));
		assertTrue(itemVendaRepository.listarItensVendaPorCodigoVenda(venda.getCodigo()).isEmpty());
		venda.getItensVenda().forEach(i -> assertNotNull(discoRepository.findById(i.getDisco().getCodigo())));
		venda.getItensVenda().forEach(i -> assertNotNull(cashbackRepository.findById(i.getCashback().getCodigo())));
	}
